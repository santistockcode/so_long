...
/*   Updated: 2024/12/27 14:10:55 by saalarco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (i < map_data->height)
    {
        ...
        while (j < map_data->width)
        {
            if (map_data->cells[i][j] != WALL && map_data->cells[i][j] != NOWALL
                && map_data->cells[i][j] != PLAYER && map_data->cells[i][j] != EXITMAP
...
static int check_has_walls(t_parsed_map *map_data)
{
    ...
    i = 0;
    j = 0;
    map_data->width = ft_strlen(map_data->cells[0]);
    while (i < map_data->height)
    {
        j = 0;
        ...
        {
            if(map_data->cells[i][0] != WALL || map_data->cells[i][map_data->width - 1] != WALL)
                return (FALSE);
        }
        i++;
    }
...
t_parsed_map *parse_map(t_file *file)
{
    ...
    {
        free_map_data_no_collectables(map_data);
        return (NULL);
    }
    return (map_data);
}
