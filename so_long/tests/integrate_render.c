...
void mock_load_images(t_game *game)
{
    ...
    if (!game->player_image || !game->nowall_image || !game->wall_image || !game->collectible_image || !game->exit_image)
...
void mock_load_textures(t_game *game)
{
    ...
    if (!game->player_texture || !game->nowall_texture || !game->wall_texture || !game->collectible_texture || !game->exit_texture)
